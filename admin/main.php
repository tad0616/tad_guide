...
/*-----------引入檔案區--------------*/
$xoopsOption['template_main'] = "tad_guide_adm_main.html";
...
function list_all_modules(){
  ...
  while(list($act_kind, $dirname, $act_name, $act_date, $cate_sn)=$xoopsDB->fetchRow($result)){
    ...
    if($last_update > $last_import)$act_date="";
    $log[$dirname][$act_kind][$cate_sn]=$act_date;
  }

  foreach($all_data as $i=>$mod){
    $dirname=$mod['dirname'];
    if(!in_array($dirname,$school_mod_arr))continue;
...
function get_dir_log($dirname,$mid){
  ...
  if(!in_array($dirname,$school_mod_arr))return;;
...
function group_cate($dirname,$mid=0){
  ...
  $sql = "select `groupid`, `name` from ".$xoopsDB->prefix("groups")." where groupid >3 order by groupid";
  $result = $xoopsDB->query($sql) or redirect_header($_SERVER['PHP_SELF'],3, mysql_error());
...
function restore_config($dirname="",$mid=""){
  ...
  if(!empty($backup_content)){
    ...
    foreach($restroe_sql as $sql){
      if(!empty($sql))$xoopsDB->queryF($sql) or die(mysql_error()."<hr>".$sql);
    }
  ...
  if($act_kind=="blocks" or $act_kind=="my_blocks"){
    ...
    while($tad_guide_data=$xoopsDB->fetchArray($result)){
      ...
      for($g=1;$g<=3;$g++){
        ...
        $rr=$xoopsDB->queryF($sql) or die($sql."<br>".mysql_error());
      ...
      $r=$xoopsDB->queryF($sql) or die($sql."<br>".mysql_error());
